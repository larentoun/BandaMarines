...
/datum/controller/subsystem/central/proc/verify_in_discord(client/player)
	var/endpoint = "[CONFIG_GET(string/central_api_url)]/oauth/token?ckey=[player.ckey]"
	var/list/headers = list(
		"Authorization" = "Bearer [CONFIG_GET(string/central_api_token)]"
	)
...
/datum/controller/subsystem/central/proc/verify_in_discord_callback(client/player, datum/http_response/response)
	...
	var/login_endpoint = "[CONFIG_GET(string/central_api_url)]/oauth/login?token=[data]"

	to_chat(player, SPAN_BOLDNOTICE("Авторизуйтесь в открывшемся окне и ожидайте 30 секунд.<br/>Если окно не открывается, можете открыть ссылку в браузере самостоятельно:<br/><a href='[login_endpoint]'>Привязать дискорд.</a>."))
